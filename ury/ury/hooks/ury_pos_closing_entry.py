...
def sub_pos_close_check(doc, method):
    # Skip multi-cashier checks for non-restaurant profiles
    if not is_restaurant_profile(doc.pos_profile):
        return
    ...
    if multiple_cashier:
        ...
        if frappe.session.user != cashier:
            ...
            if flag == 1:
                frappe.throw(
                    ("Sub Cashier POS must be closed"),
                    title=("Sub Cashier POS Closing Required")
                )
            return flag

def calculate_closing_amount(doc, method):
    # Skip multi-cashier calculations for non-restaurant profiles
    if not is_restaurant_profile(doc.pos_profile):
        return
...
def is_restaurant_profile(pos_profile):
    """Check if POS Profile is linked to a restaurant"""
    restaurant = frappe.db.get_value("POS Profile", pos_profile, "restaurant")
    return restaurant is not None and restaurant != ""